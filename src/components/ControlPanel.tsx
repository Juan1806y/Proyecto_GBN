      ...
      <Panel title="Parámetros" subtitle="Configuración del protocolo y del canal">
        <div className="space-y-4">
          <Slider
            ...
            hint={
              <>
                Número máximo de tramas que el emisor puede tener <strong>enviadas y sin
                confirmar</strong> al mismo tiempo. Es la anchura de la ventana de envío.{' '}
                {state.config.windowSize === 1
                  ? 'Con N = 1, Go-Back-N degenera en parada y espera (stop-and-wait).'
                  : `Exige al menos m = ${seqBits} bits de número de secuencia, porque N ≤ 2^m − 1.`}
              </>
            }
          />
          <Slider
            label="Temporizador de retransmisión (T_out)"
            ...
            hint={
              <>
                Tiempo que el emisor espera el ACK de la trama <em>base</em> antes de darla por
                perdida y retransmitir toda la ventana. Debe cumplirse T_out &gt; RTT = 2·t_p ={' '}
                {((state.config.propagationDelay * 2) / 1000).toFixed(2)} s.
                {rttTooLong && (
                  <span className="mt-1 block text-rose-600">
                    ⚠ Ahora es menor que el RTT: habrá timeouts prematuros y retransmisiones
                    inútiles.
                  </span>
                )}
              </>
          ...
          />
          <Slider
            label="Retardo de propagación (t_p)"
            ...
            hint={
              <>
                Tiempo que tarda una trama en recorrer el canal de un extremo al otro:{' '}
                <span className="font-mono">t_p = d / v</span> (distancia dividida por la velocidad
                de propagación de la señal). Cuanto mayor es t_p, más lento es el canal. Ida y
                vuelta: RTT = 2·t_p = {((state.config.propagationDelay * 2) / 1000).toFixed(2)} s.
              </>
            }
          />
          <Slider
            ...
            hint="Ritmo al que se reproduce la animación. Es solo un control de vídeo: no altera la lógica del protocolo ni los valores de N, T_out o t_p."
          />
        </div>
      </Panel>
